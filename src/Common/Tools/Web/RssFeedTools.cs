...
public static class  RssFeedTools
{
    ...
    static RssFeedTools()
    {
        _httpClient.DefaultRequestHeaders.Add("User-Agent", "Ai-Coreutils/1.0");
        _httpClient.Timeout = TimeSpan.FromSeconds(30);
    }

    [Description("Read articles from an RSS or Atom feed. Returns the latest articles with titles, descriptions, and links.")]
    public static async Task<string> ReadRssFeed(
    ...
    [Description("Get news from popular RSS feeds. Supported sources: bbc, cnn, reuters, techcrunch, hackernews, guardian, nytimes, reddit.")]
        ...
        [Description("Maximum number of articles to return (default: 5, max: 15).")] int maxArticles = 5)
    {
        ...
        return await ReadRssFeed(feedInfo.Url, maxArticles);
    }

    [Description("Search for RSS feeds related to a topic. Returns a list of potential RSS feed URLs.")]
    public static async Task<string> FindRssFeeds(
    ...
    /// <summary>
    /// Get structured news articles from a source. Used by BackgroundAgent for SignalR.
    /// </summary>
